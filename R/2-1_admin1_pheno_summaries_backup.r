...
# Load Phenology Data ----
  ...
  iso3_choices<-tstrsplit(basename(files),"_",keep=1)

for(k in 1:length(iso3_choices)){

  iso3_selected<-iso3_choice[k]
...

  {
    ...
    template_xyz<-unique(merge(dat[,"pixel"],coords_index,all.x=T))[,pixel:=NULL]
  ...
  # Step 1: Choose Best Available Season Source Per Row ----
  {
      ...
      dat[!is.na(sos), season_harmonized :=  split_seasons_cpt(x = sos, max_seasons = 2L), by = admin1_name]
    ...
    #                              Lower values = stricter (more fails).
  {
    ...
    # min_n: Minimum sample size per zone (zone_id + season_harmonized) to compute
  ...
  # Step 3: Labels ----
  {
    ...
    if(season_order[1]==2){
      dat[,season_harmonized:=fifelse(season_harmonized==1L,2L,1L)]
    }
  ...
  # Create pixel-level maps ----
  {
    ...
    focal_dat_avg_long <- melt(
      focal_dat_avg[,!c("failed_seasons", "weak_seasons", "mismatch", "no_detect","ndvi_weak","s_long_p")],
  ...
  # Save outputs
  {
    output_dir <- dirs$output_rasters
    ...
    for (season_name in names(raster_list)) {
      ...
      for (i in seq_len(terra::nlyr(r_stack))) {
        ...
        r_layer <- r_stack[[i]]
        output_path <- file.path(output_dir, paste0(iso3_selected, "_", layer_name, ".tif"))
