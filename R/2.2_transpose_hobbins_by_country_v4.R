...
CHIRPS_dir<-"D:/datasets/chirps/raw"
Data_dir<-"D:/datasets/hobbins_ref_et/raw"

SaveDir<-"D:/datasets/hobbins_ref_et/raw"
CHIRPS_Dekad_Dir<-"D:/datasets/hobbins_ref_et/intermediate/array_countries"
...
if(T){
  ...
  for(i in 1:length(FILES$FullPath)){
    ...
    R.utils::gunzip(FILE,remove=F,temporary=F,overwrite=T)
  }
...
FILES<-FILES[!grepl("_1980|tif.gz",FileName)]

FullPath<-list.files(Data_dir,".tif",full.names = T)
FullPath<-FullPath[!grepl("_1980|tif.gz",FullPath)]

FILES[,Year:=as.numeric(substr(FileName,5,5+3))
      ][,Dekad:=as.numeric(substr(FileName,9,10))
        ][,File:=FullPath]
...
Countries<-as.character(sort(AfricaMap$ADMIN))
